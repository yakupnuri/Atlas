...
export default function ANBIPage() {
  ...
  const documentCards = [
    {
      id: 'beloningsbeleid',
      title: t('anbi.beloningsbeleid.title'),
      description: t('anbi.beloningsbeleid.description'),
      icon: FileText,
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="bg-blue-50 border-l-4 border-blue-500 p-6 rounded-r-lg">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">{t('anbi.info.title')}</h2>
              <p className="text-gray-700 leading-relaxed mb-4">
                {t('anbi.info.description')}
              </p>
              <div className="grid md:grid-cols-2 gap-4 mt-6">
                <div className="bg-white p-4 rounded-lg shadow-sm">
                  <h3 className="font-semibold text-gray-900 mb-2">{t('anbi.info.fiscalTitle')}</h3>
                  <p className="text-gray-600 text-sm">{t('anbi.info.fiscalDesc')}</p>
                </div>
                <div className="bg-white p-4 rounded-lg shadow-sm">
                  <h3 className="font-semibold text-gray-900 mb-2">{t('anbi.info.transparencyTitle')}</h3>
                  <p className="text-gray-600 text-sm">{t('anbi.info.transparencyDesc')}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <motion.div 
            ...
            transition={{ duration: 0.6 }}
          >
            <h2 className="text-3xl font-bold text-gray-900 mb-4">{t('anbi.documents.title')}</h2>
            <p className="text-gray-600 max-w-2xl mx-auto">{t('anbi.documents.subtitle')}</p>
          </motion.div>

          {loading ? (
            <div className="text-center py-12">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#05B6C4] mx-auto"></div>
              <p className="mt-4 text-gray-600">{t('anbi.loading')}</p>
            </div>
          ) : (
